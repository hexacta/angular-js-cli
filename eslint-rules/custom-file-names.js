 ...
 * The option 'ignoreAppRoot' ignores the modules 'app.' for a module's name
 * 
...
module.exports = {
    ...
    create: (function() {
        ...
        var filenameUtil = {
            ...
            ignoreAppRoot: function(name, options){
            ...
            removePrefix: function(name, options) {
                var regName = '^' + options.ignorePrefix.replace(/[\.]/gm, '\\$&');
                var replaceName = regName;
                var firstLetterReg = options.ignorePrefix.indexOf('\.') === -1 ? '\.[A-Z]' : '\.[a-zA-z]';
                if (new RegExp(regName.replace(/\./g, firstLetterReg)).test(name)) {
                    return this.firstToLower(name.replace(new RegExp(replaceName), ""));
                }
                return name;
            },
            ...
            createExpectedName: function(name, type, options) {
                ...
                if (options.ignoreAppRoot && type === 'module') {
                    name = filenameUtil.ignoreAppRoot(name, options);
                }
